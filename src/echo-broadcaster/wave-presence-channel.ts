...
export default class WavePresenceChannel
  extends WavePrivateChannel
  implements PresenceChannel
{
  ...
  constructor(connection, name, options) {
    ...
    this.joinRequest = authRequest(name, connection, {
      ...this.options,
      authEndpoint:
        (this.options.host ?? "") +
    ...
    this.joinRequest.response.catch((error) => {
