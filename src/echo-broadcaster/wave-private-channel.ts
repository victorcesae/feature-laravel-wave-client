...
export default class WavePrivateChannel extends WaveChannel {
  ...
  constructor(connection, name, options) {
    ...
    this.auth.response.catch((error) =>
  ...
  public whisper(eventName, data) {
    request(this.connection)
      .post(
        (this.options.host ?? "") + this.options.endpoint + "/whisper",
        this.options,
        { channel_name: this.name, event_name: eventName, data }
      )
      .catch((error) =>
        this.errorCallbacks.forEach((callback) => callback(error))
      );
  ...
  public stopListeningForWhisper(
    event: string,
    callback?: Function
  ): WavePrivateChannel {
    if (callback) {
